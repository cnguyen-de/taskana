...
class MonitorControllerRestDocumentation extends BaseRestDocumentation {
  ...
  void getTaskStatusReport() throws Exception {
    this.mockMvc
        .perform(
            RestDocumentationRequestBuilders.get(restHelper.toUrl(RestEndpoints.URL_MONITOR_TASKS_STATUS))
  ...
  void getTimestampReport() throws Exception {
    this.mockMvc
        .perform(
            RestDocumentationRequestBuilders.get(restHelper.toUrl(RestEndpoints.URL_MONITOR_TIMESTAMP))
                .accept("application/hal+json")
                .header("Authorization", ADMIN_CREDENTIALS))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andDo(
